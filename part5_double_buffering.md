### 1.6 TensorOJ 实战：Add Medium 的双缓冲流水线
...
##### 1.6.2.1 核心计算单元：AIC、AIV 与 Scalar
...
#### 1.6.4 双缓冲（Double Buffering）机制：片上内存的乒乓重叠
...
##### 1.6.4.2 片上内存分配：`TPipe::InitBuffer` 的双缓冲控制
...
##### 1.6.4.4 时间段 1：预热准备阶段（CopyIn-Tile0）
...
##### 1.6.4.5 时间段 3：稳定重叠峰值阶段（CopyOut-Tile0 / Compute-Tile1 / CopyIn-Tile2）
...
##### 1.6.4.6 时间段 5：收尾清空阶段（CopyOut-Tile2）
