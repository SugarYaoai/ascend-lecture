...
在常规的单线程控制流中，算子通常遵循“数据搬入 $\to$ 向量计算 $\to$ 结果写回”的严格顺序。这种安排会造成明显的硬件等待：

- **Vector 计算单元等待搬运**：搬运引擎正从 GM 将数据送入片上 SRAM 时，Vector 单元因缺少输入而闲置。
- **数据搬运引擎等待计算**：Vector 单元执行加法时，搬运引擎同样处于等待状态。

AI Core 的设计目标，是让相互独立的硬件单元持续工作，而不是按部就班地相互等待。只要前后指令不存在强制的数据依赖，后一 Tile 的搬运指令就可以在前一 Tile 的计算尚未结束时提前发射并执行。

#### 二、AI Core 的硬件划分与数据搬运通路

AI Core 将计算与存储资源划分为不同模块，其中包括面向矩阵计算的 AIC 与面向向量计算的 AIV。Add 算子位于向量计算通路，因此重点关注 GM、UB、MTE 与 Vector 单元之间的关系。
...
- **计算单元**：Scalar 单元负责控制流与标量运算；Vector 单元负责向量运算；Cube 单元负责矩阵运算。
- **片上存储**：在向量计算通路中，核心片上存储是 Unified Buffer（UB）。Vector 与 Scalar 单元可直接读写 UB。
- **数据搬运引擎**：MTE2 负责将输入从全局内存（GM）搬入片上存储；MTE3 负责将片上计算结果从 UB 写回 GM。
...
在 Add 使用的流水线标识中，重点涉及：

- **标量队列 `PIPE_S`**：执行标量与控制指令。
- **向量队列 `PIPE_V`**：执行 UB 上的向量加法。
- **搬入队列 `PIPE_MTE2`**：将输入 Tile 从 GM 异步搬入 UB。
- **写回队列 `PIPE_MTE3`**：将输出 Tile 从 UB 异步写回 GM。
