...
异步指令队列赋予了硬件各自独立推进的能力，但**它只负责指令调度，并不自动保证数据依赖与安全性**。

如果软件层没有做好物理隔离，当 Scalar 单元迅速发射了下一轮 Tile 的 MTE2 搬运指令后，MTE2 引擎就会立即开始搬运；而此时 Vector 单元可能还在处理当前 Tile 的加法运算。若两者指向同一个片上 UB 缓冲区，就会发生严重的数据覆盖与脏读报错。

因此，异步指令流提供了硬件层面的重叠能力，而**双缓冲（Double Buffering）技术**则是解决其数据竞争、释放其全部算力潜能的软件钥匙。
