...
#### 三、异步指令流：解耦控制与执行的硬件多跑道

普通的 CPU 程序本质上是**单线式的顺序执行**：一条指令必须等待前一条指令完全执行完毕或退役，CPU 才会推进后续逻辑。

而昇腾 AI Core 的核心突破在于设计了**控制流与执行流分离的异步指令流架构**。

##### （一）Scalar 极速发射与“发射即离开”

Scalar 单元在 AI Core 中扮演着**调度总指挥**的角色。它不等待耗时的数据搬运或复杂的向量加法，而是以极高的频率顺序解析 Kernel 指令。

一旦 Scalar 发现某条指令属于数据搬运，它会立刻将指令投递到 `PIPE_MTE2` 队列；紧接着发现下一条指令是向量计算，又会瞬间将其投递到 `PIPE_V` 队列。这种“分发即离开（Fire-and-Forget）”的发射机制，使控制流能够远远走在硬件计算的前头。

##### （二）多指令队列的独立并行推进

被 Scalar 发射出去的指令，会分别进入各自硬件模块的专属 FIFO（先入先出）队列中：
...
对于本节的 Add，最关键的并发组合是：`PIPE_MTE2` 搬入 Tile `i + 1`，`PIPE_V` 计算 Tile `i`，`PIPE_MTE3` 写回 Tile `i - 1`。这三条指令队列在硬件层面物理解耦、互不干扰：MTE2 搬运引擎处理 `PIPE_MTE2` 中的搬运任务时，Vector 单元可以同时执行 `PIPE_V` 中的计算任务。这就是异步指令流实现数据搬运与计算并发重叠的根本原因。

##### （三）异步指令流带来的数据竞争矛盾

异步指令队列赋予了硬件各自飞奔的能力，但**它只管指令发射，不管数据安全**。

如果软件层没有做好隔离，当 Scalar 单元迅速发射了下一轮 Tile 的 MTE2 搬运指令后，MTE2 引擎就会立即执行搬运；此时 Vector 单元可能还在处理上一轮 Tile 的加法运算。若两者指向同一个片上 UB 缓冲区，就会发生严重的数据覆盖与脏读。

因此，异步指令流提供了并发的可能性，而**双缓冲（Double Buffering）技术**则是解决其数据竞争、释放其全部算力潜能的关键钥匙。
