...
存储层级之所以需要显式管理，是因为数据在不同位置之间并不会自动流动。AI Core 内部至少有两类与本例直接相关的执行单元：

- **MTE（Memory Transfer Engine）**：内存搬运单元，负责 GM 与 UB 之间的数据传输。它把一段连续数据从一个位置搬到另一个位置，关注的是地址与传输的字节数。
- **Vector Unit**：向量计算单元，负责在 UB 中对一批同类型元素执行加法等 SIMD 运算。它读取 UB 中的 `x`、`y`，将计算得到的 `z` 写入 UB。

因此，Add 的执行不是“直接对 GM 中的两个向量相加”，而是 MTE 先将当前片段搬入 UB，Vector Unit 在 UB 中计算，再由 MTE 将结果写回 GM。MTE 与 Vector Unit 是独立的硬件流水线；本节的代码会用同步保证它们在存在数据依赖时按正确顺序交接。

##### 2.2.3 本例的 UB 容量与对齐检查

本例中，每个 Block 要在 UB 中同时暂存 `x`、`y` 和 `z` 的一段数据：

| UB 缓冲区 | 元素个数 | 单元素大小 | 占用 | 长度的 32B 校验 |
| --- | ---: | ---: | ---: | --- |
| `x_local` | 10752 | 4 B | 43008 B | `43008 / 32 = 1344` |
| `y_local` | 10752 | 4 B | 43008 B | `43008 / 32 = 1344` |
| `z_local` | 10752 | 4 B | 43008 B | `43008 / 32 = 1344` |
| 合计 | 32256 | - | 129024 B（126 KB） | - |

三段缓冲区合计 `126 KB`，低于单个 AI Core 约 `256 KB` 的 UB 容量，并为运行时所需空间保留了余量。单段长度 `43008 B` 是 `32 B` 的整数倍；本例的 Block 偏移同样是 `43008 B` 的整数倍，因此只要 GM 分配得到的基地址满足对齐，所有 Block 的起始地址都会保持对齐。容量与对齐共同决定了这份固定规格能否直接使用一整段 UB 缓冲区完成 Add。
